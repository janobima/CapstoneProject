...
# File: Step4.2_av_fusion_reduced_facial_spect_gated.py
# AV + Spectrogram (Reduced) + Face (CACHED) with GATING 
...
#   - Early fusion: 100+32+100+64 = 296
#
# Add Face 
#   - Uses cached arrays:
#       face_cache_author_T100_vids.npy
#       face_cache_author_T100_face1.npy  (N,100,256) float16
#       face_cache_author_T100_face2.npy  (N,100,704) float16
#   - Pool per clip: mean + std over time for each (T,dim)
#       face1 pooled: 256*2 = 512
#       face2 pooled: 704*2 = 1408
#       total face pooled dim = 1920
#   - Standardize face pooled using TRAIN stats only
#
# Gating :
#   - Build AVS fused vector z_avs (296)
#   - Build face embedding f_face (projected to FACE_OUT, default 128)
#   - Compute gate g = sigmoid(W_gate(z_avs))  (vector gate, size FACE_OUT)
#   - Use gated face: g * f_face
#   - Final regressor input: [z_avs , g*f_face]
#
# Training:
#   - Early stopping on VALID MAE
#   - Multi-seed (default 5 seeds)
#   - Saves predictions + metrics per split per seed + summary mean±std
# -------------------------------------------------------------------
...
# Existing face cache 
FACE_VIDS_NPY = os.path.join(FEAT, "face_cache_author_T100_vids.npy")
FACE1_NPY = os.path.join(FEAT, "face_cache_author_T100_face1.npy")
FACE2_NPY = os.path.join(FEAT, "face_cache_author_T100_face2.npy")

OUT_DIR = os.path.join(RESULTS, "avs_fusion_reduced_multiseed_gated_face")
...
# Face gating settings
FACE_POOL_DIM = 1920      # (256 mean+std) + (704 mean+std) = 512 + 1408 = 1920
FACE_OUT_DIM = 128        # projected face embedding size 
GATE_DROPOUT = 0.10       #  dropout on gate inputs
FACE_EMB_DROPOUT = 0.10   #  dropout on face embedding
...
# ---------------- Face cache loading + pooling ----------------
def must_exist(path: str):
    if not os.path.exists(path):
        raise FileNotFoundError(f"Missing: {path}")

def load_face_cache_memmap():
    """
    Loads your existing author memmap cache (float16 arrays).
    Returns:
      vids_list: list[str]
      face1_mm: np.memmap (N,100,256) float16
      face2_mm: np.memmap (N,100,704) float16
      vid_to_idx: dict[str,int]
    """
    must_exist(FACE_VIDS_NPY)
    must_exist(FACE1_NPY)
    must_exist(FACE2_NPY)

    vids_list = np.load(FACE_VIDS_NPY, allow_pickle=True).tolist()
    face1_mm = np.load(FACE1_NPY, mmap_mode="r")
    face2_mm = np.load(FACE2_NPY, mmap_mode="r")
    vid_to_idx = {v: i for i, v in enumerate(vids_list)}
    return vids_list, face1_mm, face2_mm, vid_to_idx

def pool_face_mean_std(face_seq: np.ndarray) -> np.ndarray:
    """
    face_seq: (T,D) float16/float32
    Returns: (2D,) float32 [mean, std]
    """
    x = face_seq.astype(np.float32)
    mu = x.mean(axis=0)
    sd = x.std(axis=0)
    return np.concatenate([mu, sd], axis=0).astype(np.float32)

def build_face_pooled_for_vids(
    vids: List[str],
    face1_mm,
    face2_mm,
    vid_to_idx: Dict[str, int]
    ...
    """
    Builds pooled face vectors for a list of vids using the cached memmaps.
    Output: (N,1920) float32
    """
    out = np.zeros((len(vids), FACE_POOL_DIM), dtype=np.float32)
    for i, vid in enumerate(vids):
        if vid not in vid_to_idx:
            # if a vid isn't in cache (shouldn't happen if cache built on all vids),
            # keep zeros (means "no face")
            continue
        fi = vid_to_idx[vid]
        f1 = face1_mm[fi]  # (T,256)
        f2 = face2_mm[fi]  # (T,704)
        p1 = pool_face_mean_std(f1)  # 512
        p2 = pool_face_mean_std(f2)  # 1408
        out[i] = np.concatenate([p1, p2], axis=0)
    return out
...
class AVSReducedGatedFaceRegressor(nn.Module):
    """
    z_avs = concat(w,s,v,sp)  -> 296
    face_raw (1920) -> face_emb (FACE_OUT_DIM)
    gate = sigmoid(W_gate(z_avs)) -> (FACE_OUT_DIM)
    z = concat(z_avs, gate * face_emb)
    """
    def __init__(
        self,
        w2v_in: int,
        ...
        sf_out: int = 100,
        spec_out: int = 64,
        face_in: int = FACE_POOL_DIM,
        face_out: int = FACE_OUT_DIM,
    ):
        super().__init__()
        ...
        self.avs_dim = w2v_out + smile_out + sf_out + spec_out  # 296

        # Face branch
        self.face_proj = nn.Sequential(
            nn.Linear(face_in, face_out),
            nn.ReLU(),
            nn.LayerNorm(face_out),
            nn.Dropout(FACE_EMB_DROPOUT),
        )

        # Gate from AVS only (vector gate)
        self.gate = nn.Sequential(
            nn.Dropout(GATE_DROPOUT),
            nn.Linear(self.avs_dim, face_out),
        )

        self.regressor = MLPRegressor(self.avs_dim + face_out)

    def forward(self, x_wav, x_smile, x_sf, x_spec, x_face):
        w = self.wav_proj(x_wav)
        s = self.smile_proj(x_smile)
        v = self.sf_proj(x_sf)
        sp = self.spec_enc(x_spec)

        z_avs = torch.cat([w, s, v, sp], dim=1)  # (B,296)

        face_emb = self.face_proj(x_face)        # (B,face_out)
        gate = torch.sigmoid(self.gate(z_avs))   # (B,face_out)

        z = torch.cat([z_avs, gate * face_emb], dim=1)
        return self.regressor(z)

# ---------------- Dataset ----------------
class AVSFaceDataset(Dataset):
    def __init__(
        self,
        vids: List[str],
        y: np.ndarray,
...
def collate_fn(batch):
    ...
    xf = torch.tensor(np.stack([b[5] for b in batch]), dtype=torch.float32)
    y = torch.tensor([b[6] for b in batch], dtype=torch.float32)
    return vids, xw, xs, xv, xsp, xf, y
...
def build_splits():
    ...
    # load face cache once
    _, face1_mm, face2_mm, vid_to_idx = load_face_cache_memmap()
    ...
    def build_split(split_name: str):
        ...
        # pooled face from cached memmaps 
        Xf = build_face_pooled_for_vids(vids, face1_mm, face2_mm, vid_to_idx)  # (N,1920)

        return vids, Xw, Xs, Xv, Xf, y, spec_paths

    train = build_split("train")
    valid = build_split("valid")
    test  = build_split("test")

    dims = {"w2v_dim": w2v_dim, "sm_dim": sm_dim, "sf_dim": sf_dim, "face_dim": FACE_POOL_DIM}
...
def predict(model, loader):
    ...
    for vids, xw, xs, xv, xsp, xf, y in loader:
        xw = xw.to(DEVICE)
        xs = xs.to(DEVICE)
        xv = xv.to(DEVICE)
        xsp = xsp.to(DEVICE)
...
def train_one_seed(seed: int, train_data, valid_data, test_data, dims: Dict[str, int]) -> Dict[str, float]:
    ...
    train_vids, Xw_train, Xs_train, Xv_train, Xf_train, y_train, sp_train = train_data
    valid_vids, Xw_valid, Xs_valid, Xv_valid, Xf_valid, y_valid, sp_valid = valid_data
    test_vids,  Xw_test,  Xs_test,  Xv_test,  Xf_test,  y_test,  sp_test  = test_data

    # Standardize wav/smile/slowfast/face with TRAIN stats only
    ...
    mu_f, sig_f = standardize_fit(Xf_train)
    Xf_train = standardize_apply(Xf_train, mu_f, sig_f)
    Xf_valid = standardize_apply(Xf_valid, mu_f, sig_f)
    Xf_test  = standardize_apply(Xf_test,  mu_f, sig_f)

    # loaders
    train_ds = AVSFaceDataset(train_vids, y_train, Xw_train, Xs_train, Xv_train, Xf_train, sp_train)
    valid_ds = AVSFaceDataset(valid_vids, y_valid, Xw_valid, Xs_valid, Xv_valid, Xf_valid, sp_valid)
    test_ds  = AVSFaceDataset(test_vids,  y_test,  Xw_test,  Xs_test,  Xv_test,  Xf_test,  sp_test)

    train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE_TRAIN, shuffle=True,
                              num_workers=0, pin_memory=PIN_MEMORY, collate_fn=collate_fn)
    ...
    # model
    model = AVSReducedGatedFaceRegressor(
        w2v_in=dims["w2v_dim"],
        ...
        sf_out=100,
        spec_out=64,
        face_in=dims["face_dim"],
        face_out=FACE_OUT_DIM,
    ).to(DEVICE)
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        model.train()
        total_loss = 0.0
        for vids, xw, xs, xv, xsp, xf, yb in train_loader:
            xw = xw.to(DEVICE)
            xs = xs.to(DEVICE)
            xv = xv.to(DEVICE)
            xsp = xsp.to(DEVICE)
...
def main():
    for p in [INDEX_PATH, W2V_PATH, SMILE_PATH, SLOWFAST_DIR, SPEC_DIR, FACE_VIDS_NPY, FACE1_NPY, FACE2_NPY]:
        must_exist(p)
    ...
    with open(os.path.join(OUT_DIR, "config.json"), "w", encoding="utf-8") as f:
        json.dump({
            ...
            "avs_dim": 100 + 32 + 100 + 64,
            "face_pool_dim": FACE_POOL_DIM,
            "face_out_dim": FACE_OUT_DIM,
            "gate_dropout": GATE_DROPOUT,
            "face_emb_dropout": FACE_EMB_DROPOUT,
            "device": DEVICE
        }, f, indent=2)
