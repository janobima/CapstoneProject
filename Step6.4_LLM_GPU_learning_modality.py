...
# April 2026
#
# File: Step6.4_LLM_GPU_learning_modality
#
#
# Experiminting with different modalities combinations :
# - Retrieve ONLY the selected modalities (audio/visual/face/combos)
# - Prompt also includes ONLY the selected modalities
# - best similarity method used: Cosine distance over z-scored numeric vectors
# - Saves: predictions.jsonl, predictions.csv, metrics.json, failures_raw.jsonl
#
# Example:
# python Step6.4_LLM_GPU_learning_modality.py \
#   --jsonl $/gce_project/results/llm_inputs_v1_all_modalities.jsonl \
#   --labels $/gce_project/results/llm_eval_labels.csv \
#   --outdir $/gce_project/results/llm_runs/qwen3b_k16_audio_only \
#   --model Qwen/Qwen2.5-3B-Instruct \
#   --k 16 \
#   --modality_set audio \
#   --split_train train --split_test test \
#   --device cuda --dtype fp16 --batch_size 8 --max_new_tokens 120 --resume
#
# modality_set options:
#   all | audio | visual | face | audio_visual | audio_face | visual_face
...
# Modality masking
# =========================
def modality_flags(modality_set: str) -> Tuple[bool, bool, bool]:
    ms = (modality_set or "all").lower().strip()
    want_audio = ms in {"all", "audio", "audio_visual", "audio_face"}
    want_visual = ms in {"all", "visual", "audio_visual", "visual_face"}
    want_face = ms in {"all", "face", "audio_face", "visual_face"}
    return want_audio, want_visual, want_face
...
def build_feature_key_list(modality_set: str) -> List[str]:
    """
    Retrieval feature space = ONLY keys from selected modalities.
    IMPORTANT: This must match what the LLM sees (full masking).
    """
    want_audio, want_visual, want_face = modality_flags(modality_set)

    keys = ["group_context.num_people"]
...
def compact_clip_json(row: Dict[str, Any], modality_set: str = "all") -> Dict[str, Any]:
    """
      - keep ONLY selected evidence_summary sub-dicts
    """
    want_audio, want_visual, want_face = modality_flags(modality_set)
    ev = row.get("evidence_summary", {}) or {}
    ev_out: Dict[str, Any] = {}

    if want_audio:
        if "audio_smile" in ev:
            ev_out["audio_smile"] = ev.get("audio_smile", {})
        if "audio_wav2vec" in ev:
            ev_out["audio_wav2vec"] = ev.get("audio_wav2vec", {})

    if want_visual:
        if "visual_slowfast" in ev:
            ev_out["visual_slowfast"] = ev.get("visual_slowfast", {})

    if want_face:
        if "face_behavior" in ev:
            ev_out["face_behavior"] = ev.get("face_behavior", {})
...
def build_fewshot_user_content(
    ...
    for ex_row, y in train_examples:
        ex_json = compact_clip_json(ex_row, modality_set=modality_set)
        lines.append(
            f"- example_json: {json.dumps(ex_json, ensure_ascii=False)}\n"
            f"  label_y_true: {int(y)}"
        )
    examples_block = "\n".join(lines)

    query_json = compact_clip_json(query_row, modality_set=modality_set)
...
def run_batch_prompts(
    ...
    # with left padding, slice using padded prompt length
...
def main():
    ...
    ap.add_argument("--split_test", default="test")

    ap.add_argument("--k", type=int, default=16, help="neighbors per test clip")
    ap.add_argument(
        "--modality_set",
        default="all",
        choices=["all", "audio", "visual", "face", "audio_visual", "audio_face", "visual_face"],
        help="Mask BOTH retrieval vectors and prompt evidence_summary to selected modalities",
    )
    ...
    try:
        ...
        print(f"[error] Cannot create outdir: {args.outdir}")
        print(f"        PermissionError: {e}")
        print("        Fix: set --outdir inside your own home directory, e.g.")
        print("        /home/msds/maryam007/gce_project/results/llm_runs/...")
        return
    ...
    # Split rows
    train_rows = [r for r in rows if str(r.get("split", "")).lower() == args.split_train.lower()]
    test_rows = [r for r in rows if str(r.get("split", "")).lower() == args.split_test.lower()]

    # Keep only rows that have labels (examples + evaluation)
    train_rows = [r for r in train_rows if str(r.get("clip_id")) in labels_map]
    test_rows = [r for r in test_rows if str(r.get("clip_id")) in labels_map]

    print(f"Train rows with labels: {len(train_rows)} | Test rows with labels: {len(test_rows)}")
    print(f"Modality masking (retrieval+prompt): {args.modality_set} | k={args.k}")
    ...
    if args.resume:
        ...
        if done:
            ...
            print(f"[resume] Found {len(done)} completed clip_ids. Remaining test: {len(test_rows)}")

    if len(test_rows) == 0:
        print("Nothing to run.")
        return
    if len(train_rows) == 0:
        print("[error] No train rows with labels found. Check split_train and labels mapping.")
        return

    # Build retrieval vectors (FULL MASKING)
    feature_keys = build_feature_key_list(args.modality_set)
    print("Retrieval feature_keys:", feature_keys)
    ...
    X_test_imp = np.where(np.isnan(X_test), mean_, X_test)
    X_test_n = (X_test_imp - mean_) / std_
    ...
    if args.device.lower() == "cpu":
        dtype = torch.float32
    print(f"Loading model={args.model} | device={args.device} | dtype={dtype}")
    ...
    try:
        ...
        for start in range(0, len(test_rows), args.batch_size):
            ...
            for bi, qrow in enumerate(batch_rows):
                ...
                # Build K labeled examples
                examples: List[Tuple[Dict[str, Any], float]] = []
                for ti in neigh_ids:
                    tr = train_rows[int(ti)]
                    y = labels_map[str(tr.get("clip_id"))]
                    examples.append((tr, y))

                user_content = build_fewshot_user_content(
                    train_examples=examples,
                    query_row=qrow,
                    modality_set=args.modality_set,
                )
            ...
            pred_f.flush()
            fail_f.flush()
            print(f"[progress] {min(start + args.batch_size, len(test_rows))}/{len(test_rows)} written | failures={all_failures}")

    finally:
        pred_f.close()
        fail_f.close()

    # Reload preds for metrics + csv
    preds = []
    ...
    print(f"[Metrics] matched labels: {len(y_true)} / total preds: {len(preds)} | parse_failures: {n_fail}")
    ...
    if len(y_true) > 0:
        ...
        metrics = {
            "n_total_predictions": int(len(preds)),
        ...
        print("\n=== LLM Metrics (Retrieval few-shot, FULL MASKING) ===")
        print(json.dumps(llm_m, indent=2))
        print("\n=== Mean Baseline Metrics ===")
        print(json.dumps(base_m, indent=2))
    else:
        with open(metrics_path, "w", encoding="utf-8") as f:
            json.dump({"error": "No ground-truth labels matched clip_ids."}, f, indent=2)
        print("Wrote:", metrics_path)
