...
# File: Step4.1_av_fusion_reduced_spect.py
# -------------------------------------------------------------------
# AV + Spectrogram (Reduced) / No Face
...
OUT_DIR = os.path.join(RESULTS, "avs_fusion_reduced_multiseed")
...
def load_spec_image(vid: str, reshape=(128, 512)) -> np.ndarray:
    path = find_spec_path(vid)
    img = Image.open(path).convert("RGB")
    img = img.resize((reshape[1], reshape[0]))  # (W,H)
...
class AVSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        vid = self.vids[idx]
        # Load spectrogram on-the-fly (keeps RAM low)
