...
# File: Step6.2_LLM_GPU_learning.py
#
# Retrieval few-shot evaluation:
# - Uses ALL train samples to get the closest K samples
...
# Few-shot wrapper: K labeled examples + query
...
def to_num(v: Any) -> float:
    """Convert value to numeric for retrieval vectors."""
    if v is None:
        return np.nan
...
def flatten_fields_for_vector(row: Dict[str, Any], feature_keys: List[str]) -> np.ndarray:
    """
    Take a fixed list of keys and turn row -> numeric vector.
    Missing -> nan.
    """
    vec = []
    for k in feature_keys:
        vec.append(to_num(get_by_path(row, k)))
...
def build_feature_key_list(rows: List[Dict[str, Any]]) -> List[str]:
    """
    Define retrieval feature space from the known columns in the JSON structure.
    """
    keys = [
        "group_context.num_people",
        # "group_context.interaction_keyword",
...
def impute_and_normalize(X: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    # if a column is all-nan, nanmean -> nan; replace with 0
...
def compact_clip_json(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # reference_note is long; include only binning rule if present
...
def main():
    ...
    # Keep only rows that have labels (critical for examples)
