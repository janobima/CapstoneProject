...
# File: Step3.1_av_fusion_w2v_smile_slowfast.py
# Description: This code combines FULL (non reduced) audio features (wav2vec and OpenSMILE)
# with visual features (SlowFast) to predict group cohesion scores. It trains
# a model on the fused features and evaluates performance using regression metrics.
# ============================================================
import os
import glob
import numpy as np
...
PRED_OUT = os.path.join(RESULTS, "av_fusion_w2v_smile_slowfast_predictions.csv")
METRICS_OUT = os.path.join(RESULTS, "av_fusion_w2v_smile_slowfast_metrics.txt")
...
def load_slowfast_vec(vid: str) -> np.ndarray:
    """
    Expected file: features/slowfast/<vid>.npz  e.g., VID_6_12.npz
    Expected key: 'feat' (per authors' guide). We'll handle fallback keys robustly.
    """
    path = os.path.join(SLOWFAST_DIR, f"{vid}.npz")
    if not os.path.exists(path):
        # some datasets store with different extension or naming; try glob fallback
        matches = glob.glob(os.path.join(SLOWFAST_DIR, f"{vid}*.npz"))
        if not matches:
            raise FileNotFoundError(f"SlowFast file not found for vid={vid} at {path}")
        path = matches[0]
    ...
    feat = np.array(feat)
    # Paper guide suggests feat[0, :] shape; handle common shapes robustly
    if feat.ndim == 2:
        # could be (1, D) or (T, D). If (T, D), average over time.
        if feat.shape[0] == 1:
            vec = feat[0]
        else:
...
def main():
    ...
    # 4) Cache SlowFast vectors (fast + avoids repeated disk reads)
    unique_vids = index_df["vid"].unique().tolist()
    slowfast_cache = {}
    missing_sf = 0
    ...
    if missing_sf > 0:
        print(f"Warning: SlowFast missing for {missing_sf} vids. Those samples will be skipped.")

    # Determine SlowFast dim from any cached entry
    any_vec = next(iter(slowfast_cache.values()))
    slowfast_dim = int(any_vec.shape[0])
    print("Detected SlowFast dim:", slowfast_dim)

    # 5) Build fused X,y per split
    def build_split(split_name: str):
        ...
        X, y, keep = [], [], []
        for v in vids:
            if (v in w2v_map) and (v in smile_map) and (v in slowfast_cache):
                x_audio = np.concatenate([w2v_feat[w2v_map[v]], smile_feat[smile_map[v]]], axis=0)
                x_av = np.concatenate([x_audio, slowfast_cache[v]], axis=0)
                X.append(x_av)
    ...
    metrics_text = f"""
Audio-Visual Fusion Baseline (wav2vec + OpenSMILE + SlowFast) - MLP Regressor
----------------------------------------------------------------------------
Samples: train={len(train_vids)}, valid={len(valid_vids)}, test={len(test_vids)}
Input dim: {X_train.shape[1]}  (audio + slowfast_dim={slowfast_dim})
